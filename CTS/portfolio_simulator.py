...
def compute_trading_statistics(tradingReturn,typeOfPosition,crypto):
    ...
    print("Succesfull trades: ",success,"Percentage on total: ",success_percentage,"%\n")
...
def compute_portfolio_statistics(df,classifier,labels):
    ...
    print("number of day:",day_counter,"avg allocations:",tot_allocations/day_counter)
    
    dfres = pd.DataFrame()
    dfres['Date'] = days
    dfres['Equity'] = equities
    dfres.to_excel("./data/portfolio_simulations_equity_trend/"+str(classifier)+"_"+str(labels)+"_equity.xlsx", index = False)
...
def main():
    ...
    print("Final equity: ",compute_portfolio_statistics(dfres,args.classifier,args.labels))
    dfres.to_csv("./data/portfolio_simulations_results/results_"+str(args.classifier)+"_"+str(args.labels)+".csv",index=False)
